...
function parseExtraTargets(rawTargets: string | undefined) {
...
function findSignTool() {
  ...
  return path.join(windowsKitsRoot, latest, "x64", "signtool.exe");
}

async function runPowerShellAuthenticodeSigning(targetPath: string) {
  const command = [
