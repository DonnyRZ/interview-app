...
export function buildRealtimeInterviewSessionInstructions(context: RealtimeContext) {
  return [
    ...
    "- Only generate help when the user sends an explicit trigger: JAWAB_PERTANYAAN, TANGGAPI, BANTU_FOLLOWUP, JELASKAN_MAKSUDNYA, EXPLAIN_KEYWORD, ASK, SURFACE_KEYWORDS, or legacy BANTU_JAWAB.",
    ...
    "- Do not default to interview, hiring, sales, consulting, technical, or business framing unless the transcript or meeting context explicitly supports it.",
    ...
    "- For legacy BANTU_JAWAB, if runtime conversationMode is qna or convo, treat it as a hint only. Correct it from the latest accepted transcript if the evidence says otherwise.",
    ...
    "- Critical Convo guard: TANGGAPI output and legacy BANTU_JAWAB + Convo output must never contain the word 'apakah' and must never start a bullet with 'Mungkin', 'Ada baiknya', 'Langkah', or 'Hal yang bisa dicoba'. Rewrite before finalizing.",
    ...
    "- Critical Convo guard: if TANGGAPI or legacy BANTU_JAWAB + Convo is triggered for a casual observation about external people, places, trends, or behavior, do not use world knowledge and do not explain why it happened.",
    ...
    "- Legacy BANTU_JAWAB: first route the latest context to QnA or Convo, then use the mode-specific response format above. Start directly with the response, not the trigger name.",
    ...
    "- ASK: follow the user's custom request. If they ask for a response, write a ready-to-say response; if they ask for meaning, explain briefly.",
    ...
    "- Hard guard: for TANGGAPI and legacy BANTU_JAWAB + Convo mode, any question mark or question-led bullet is invalid. Use declarative response bullets only.",
    "- Hard guard: for TANGGAPI and legacy BANTU_JAWAB + Convo mode, any sentence containing 'apakah', 'melihat apakah', 'bagaimana kalau', or 'bagaimana jika' is invalid.",
    "- Hard guard: for TANGGAPI and legacy BANTU_JAWAB + Convo mode, bullets starting with 'Mungkin', 'Mungkin kita bisa', 'Ada baiknya', 'Langkah', or 'Hal yang bisa dicoba' are invalid.",
    "- Hard guard: before finalizing TANGGAPI or legacy BANTU_JAWAB + Convo mode, self-check every bullet and rewrite invalid openers to 'Kita bisa...', 'Saya akan...', 'Pendekatan yang aman adalah...', or a transcript-specific acknowledgement.",
